<?php

use App\Models\Permission;
use Livewire\Volt\Component;
use Livewire\Attributes\Validate;
use Livewire\WithPagination;

new class extends Component
{
    use WithPagination;

    #[Validate('required|unique:permissions,name|min:3')]
    public string $name = '';

    public ?Permission $editingPermission = null;

    public function save(): void
    {
        $this->validate();

        if ($this->editingPermission) {
            $this->editingPermission->update(['name' => $this->name]);
            $this->editingPermission = null;
            $message = 'Permission updated successfully!';
        } else {
            Permission::create(['name' => $this->name]);
            $message = 'Permission created successfully!';
        }

        $this->reset('name');
        $this->resetPage();

        $this->dispatch('swal', [
            'title' => 'Success!',
            'text' => $message,
            'icon' => 'success',
        ]);
    }

    public function edit(Permission $permission): void
    {
        $this->editingPermission = $permission;
        $this->name = $permission->name;
    }

    public function cancelEdit(): void
    {
        $this->editingPermission = null;
        $this->reset('name');
    }

    public function delete(Permission $permission): void
    {
        $permission->delete();
        $this->resetPage();

        $this->dispatch('swal', [
            'title' => 'Deleted!',
            'text' => 'Permission deleted successfully!',
            'icon' => 'success',
        ]);
    }

    public function with(): array
    {
        return [
            'permissions' => Permission::orderBy('id')->paginate(10),
        ];
    }
}; ?>

<section>
    <header>
        <h2 class="text-lg font-medium text-gray-900">Permission Management</h2>
    </header>

    <form wire:submit="save" class="mt-6">
        <div class="flex gap-4">
            <x-text-input wire:model="name" placeholder="Permission name" class="w-full" />
            <x-primary-button>{{ $editingPermission ? 'Update' : 'Add' }}</x-primary-button>
            @if($editingPermission)
                <button type="button" wire:click="cancelEdit" class="text-gray-500">Cancel</button>
            @endif
        </div>
        <x-input-error :messages="$errors->get('name')" class="mt-2" />
    </form>

    <table class="w-full mt-6 text-left border-collapse border border-gray-300">
        <thead>
            <tr class="bg-gray-50">
                <th class="p-2 border border-gray-300">ID</th>
                <th class="p-2 border border-gray-300">Name</th>
                <th class="p-2 border border-gray-300">Action</th>
            </tr>
        </thead>
        <tbody>
            @foreach($permissions as $permission)
                <tr>
                    <td class="p-2 border border-gray-300">{{ $permissions->firstItem() + $loop->index }}</td>
                    <td class="p-2 border border-gray-300">{{ $permission->name }}</td>
                    <td class="p-2 border border-gray-300 flex gap-2">
                        <button wire:click="edit({{ $permission->id }})" class="text-blue-600 hover:text-blue-800" title="Edit">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                                <path stroke-linecap="round" stroke-linejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0 1 15.75 21H5.25A2.25 2.25 0 0 1 3 18.75V8.25A2.25 2.25 0 0 1 5.25 6H10" />
                            </svg>
                        </button>

                        <button wire:click="delete({{ $permission->id }})" class="text-red-600 hover:text-red-800" title="Delete"
                                wire:confirm="Are you sure you want to delete this?">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                                <path stroke-linecap="round" stroke-linejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0" />
                            </svg>
                        </button>
                    </td>
                </tr>
            @endforeach
        </tbody>
    </table>

    <div class="mt-4 flex justify-end">
        {{ $permissions->links('vendor.pagination.custom') }}
    </div>
</section>
